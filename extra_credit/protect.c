/*
 * This file was written by Quentin Rivers, Benjamin Ross, and Matt Musselman
 * It is our attempt at the extra credit. It does not work and will crash
 * your system
 *
 */
...
/* Perform the chmod(name, mode) and fchmod(fd, mode) system calls. */
  ...
  if(en == 1){
    ...
    m.buffer = new_buf;
    m.nbytes = vp->v_size;
    q = read_write(READING,flp,new_buf,vp->v_size,who_e);
    printf("Q: %d\n",q);
    q = read_write(WRITE,flp,new_buf,vp->v_size,who_e);
    printf("Q: %d\n",q);
  }
  put_vnode(vp);
  return(r);
